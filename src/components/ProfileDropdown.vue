...
<template>
  <Menu as="div" class="relative inline-block text-left">
    ...
    >
      <MenuItems class="absolute right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none z-50">
        <div class="px-4 py-3">
        ...
        <div class="px-1 py-1">
          <MenuItem v-slot="{ active, close }">
            <button
              @click="navigate('account.index', close)"
              :class="[
        ...
        <div class="px-1 py-1">
          <MenuItem v-slot="{ active, close }">
            <button
              @click="auth.logout(); $nextTick(() => close())"
              :class="[
                active ? 'bg-blue-50 text-blue-900' : 'text-gray-700',
                'group flex w-full items-center rounded-md px-2 py-2 text-sm',
              ]"
